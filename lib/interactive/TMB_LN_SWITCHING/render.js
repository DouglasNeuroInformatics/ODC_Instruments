...
export function render(onComplete) {
  ...
  tmbUI.onreadyUI = function () {
    ...
    if (~frame.type.indexOf('practice') && !correct) {
      errorMessage =
        'Your target was the <b>' +
        frame.judgement +
        ' ' +
        frame.target +
        '</b>:<br>' +
        (frame.response === 'R'
          ? 'you should have clicked<br>' + 'the <b>right</b> button.<br><br>'
          : 'you should have clicked<br>' + 'the <b>left</b> button.<br><br>');
      showAlert(
        errorMessage,
        'Click here to retry',
        function () {
    ...
    // stop the sequence and advise the participant
      ...
      showAlert(
        'Please respond as quickly as you can.<br><br>' + 'Click the left or right button.<br><br>',
        'Click here to retry',
        function () {
          showFrame('null');
          setTimeout(function () {
  ...
  function setFrameSequence() {
    var TestMessage;
    TestMessage = {
      begin: '<h2>Letter/Number Switching</h2>' + "<img src='images/Inst0.jpg' alt='Title'><br><br>",
      instructions: [
        '<h3>Instructions</h3>' +
