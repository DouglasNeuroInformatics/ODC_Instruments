...
export function render(done) {
  ...
  tmbUI.onreadyUI = function () {
    ...
    if (frame.type === 'practice') {
      ...
      if (tmbUI.status === 'timeout' || !correct) {
        ...
        showAlert(
          'The image pair you learned was:<br><br>' +
            "<img src='" +
            frame.probe +
            "'> " +
            "<img src='" +
      ...
      if (tmbUI.status === 'timeout') {
        ...
        showAlert(
          'You are taking too long to respond.<br>' +
            'You should click the image that goes with this:<br><br>' +
            "<img src='" +
            frame.probe +
            "' style='border: 5px solid red;'><br><br>",
          'Click here to retry',
          function () {
            showFrame('null');
            nextTrial();
  ...
  function setFrameSequence() {
    ...
    testMessage = {
      begin:
        '<h2>TMB Visual Paired Associates Memory - Test</h2>' +
