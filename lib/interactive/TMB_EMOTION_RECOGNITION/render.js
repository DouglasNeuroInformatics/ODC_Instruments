...
export function render(done) {
  ...
  tmbUI.onreadyUI = function () {
    ...
    if (tmbUI.status === 'timeout') {
      ...
      showAlert(
        'You are taking too long to respond.<br><br>' +
  ...
  function nextTrial() {
    ...
    // else the sequence is empty, we are done!
      ...
      for (var i = 0; i < respOpts.length; i++) {
        responseProportion.push(
          tmp1.filter(function (obj) {
            return obj.response === respOpts[i];
          }).length / tmp1.length
        );
      }
      maxResp = responseProportion.reduce((a, b) => Math.max(a, b), -Infinity);
      outcomes.flag_sameResponse = maxResp > 0.9 ? 1 : 0;
  ...
  function setFrameSequence() {
    ...
    // messages
    testMessage = {
      begin: '<h2>Guess the Emotion</h2>' + "<img src='images/emotions.png' alt='title image'><br><br>",
      instructions: ['<h2>Instructions:</h2>' + 'Click the emotion<br>that best matches the face.<br><br>']
    };
    ...
    // message to display
    var frameMessage = [testMessage.begin, testMessage.instructions[0]];
  ...
  function parseFile(text) {
    ...
    if (text) {
      ...
      if (errors) {
        showAlert('parseFile: Incorrect formatting at line ' + (i + 1) + '.<br><br>', '', null, '20pt');
